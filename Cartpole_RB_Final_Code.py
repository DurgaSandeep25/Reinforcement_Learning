...
#hyperparams
layers_list = [64, 64] # hidden layer size
max_iters = 20
num_episodes = 1000
discount = 0.99 # value of gamma
p_lr = 0.003 # Policy Learning Rate
v_lr = 0.01 # Value Learning Rate
    ...
    def __init__(self, input_dim, output_dim = 1, hidden_layer_size = [128], network_type = 'policy', initialization = 'random', activation = 'relu'):
      ...
      for i in range(len(hidden_layer_size)):
        ...
        if activation == 'leaky_relu':
          networkLayers.append(torch.nn.LeakyReLU(negative_slope=0.01))
        else:
...
for i in range(max_iters):
  ...
  policyNet =  PolicyAndValueNetwork(input_dim = num_states, output_dim = num_actions, hidden_layer_size = layers_list,
                                     initialization = 'HE', network_type = 'policy', activation = 'relu').to(device)
  valueNet = PolicyAndValueNetwork(input_dim = num_states, output_dim = 1, hidden_layer_size = layers_list,
                                   initialization = 'HE', network_type = 'value', activation = 'relu').to(device)
...
plt.plot(np.mean(np.array(all_iterations_rewards), axis=0), label ='Model Cartpole Reinforce with Baseline')
plt.xlabel('Episode')
plt.ylabel('Reward')
plt.legend()
plt.show()
...
imageio.mimsave('Cartpole_RB_Output.gif', frames, duration=0.1)  # Adjust duration as needed
