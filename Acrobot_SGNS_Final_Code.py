...
for i in range(20):
  ...
  trained_network, rewards = SGNS(n=5, epsilon=1, decay_rate=0.99, alpha=0.001, alpha_decay = 100, num_episodes=1000,
                                                     layer_list=[32], output=False, start = 'HE')
...
plt.plot(np.mean(df.values, axis=0), label ='Model Acrobot Semi Gradient n-step SARSA')
plt.xlabel('Episode')
plt.ylabel('Reward')
plt.legend()
plt.show()
...
game_name = 'Acrobot-v1'  # Update with your specific game
env = gym.make(game_name)
state = env.reset()
...
imageio.mimsave('Acrobot_SGNS.gif', frames, duration=0.1)  # Adjust duration as needed
